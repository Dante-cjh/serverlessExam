...
export class RestAPIStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const deleteMovieByIdFn = new lambdanode.NodejsFunction(
      ...
      const getAwardByIdFn = new lambdanode.NodejsFunction(
          this,
          "GetAwardByIdFn",
          {
              architecture: lambda.Architecture.ARM_64,
              runtime: lambda.Runtime.NODEJS_16_X,
              entry: `${__dirname}/../lambdas/getAwardsById.ts`,
              timeout: cdk.Duration.seconds(10),
              memorySize: 128,
              environment: {
    ...
    movieEndpoint.addMethod(
      ...
      const awardAllEndpoint = awardsEndpoint.addResource("{awardBody}");
      const awardEndpoint = awardAllEndpoint.addResource("movies").addResource("{movieId}");
      awardEndpoint.addMethod(
          "GET",
          new apig.LambdaIntegration(getAwardByIdFn, { proxy: true })
      )
    ...
    movieAwardsTable.grantReadData(getAwardByIdFn);
